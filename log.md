...
- Medium draft created (unpublished). Pasted as HTML; Medium re-hosted all five images from the public repo.
...
- Video live at https://youtu.be/GVSNnOFtazo (Christian retitled it "Can a local AI run on a tiny computer? Let's find out with the Cheap Yellow Display!"). Linked from the README and blog (pushed), and embedded in the Medium draft after the intro.
- Next: Christian publishes the Medium post; then add its link to the YouTube description and the README.
- Creature v5 on the board: speech bubble along the bottom (the sentence it hands Needle), full gauges always visible, reasoning under THINKING, tally in the header, footer dropped. It says only what stands out, and world events interrupt it and go into the sentence. New fine-tune (tools/make_creature5_data.py; the first try was fragile on "I'm ..." openings, fixed by adding "I'm ..." phrasings to training): 30/30 on engine and port with identical reasoning. On the board, 12 decisions in 12 minutes all matched the policy, 5 of them event reactions, 34–49 s each, 2.9 KB heap free (context raised to 36 turn tokens). Fixed the world clock double-counting the last action at the start of each decision. Mood drift 0.43 → 0.34/s from a per-second simulation.
- Images via ChatGPT: a four-panel chainsaw comic and four illustrations (elephant boundary, water pump, fire sensor, vaccine fridge) in outputs/images/. One new chat was needed after a stuck turn. Comic, new diagram and new creature sheet are in the Medium draft.
- Posts, blog drafts and images, and the video sources are no longer in the repo (docs/blog/, video/ gitignored) and have been scrubbed from the public branch history; backup ref public-before-scrub. Needs Christian's force-push.
